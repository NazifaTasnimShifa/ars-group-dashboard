...
export default function InventoryStatusPage() {
  ...
  const inventory = useMemo(() => {
    ...
    return companyInventory.filter(item => 
  ...
  return (
    <DashboardLayout>
      <Modal open={modalState.open} setOpen={(val) => setModalState({...modalState, open: val})} title={`${modalState.mode === 'add' ? 'Add' : 'Edit'} Inventory Item`}>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {inventory.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{item.stock.toLocaleString('en-IN')} {item.unit}</td>
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{formatCurrency(item.stock * item.costPrice)}</td>
