...
import {
  Bars3Icon, XMarkIcon, HomeIcon, FolderIcon, InboxIcon, UsersIcon, TruckIcon,
  ChevronDoubleLeftIcon, ArrowRightStartOnRectangleIcon, ArrowPathIcon, ChevronRightIcon
...
export default function DashboardLayout({ children }) {
  ...
  if (!selectedCompany) { return <div>Loading company...</div>; }

  const sidebarContent = (
    <div className="flex grow flex-col gap-y-5 overflow-y-auto bg-gray-900 px-6 pb-4">
      <div className="flex h-16 shrink-0 items-center">
        <h1 className={classNames(isCollapsed && 'lg:hidden', "text-2xl font-bold text-white transition-opacity duration-200")}>
      ...
      <nav className="flex flex-1 flex-col">
        <ul role="list" className="flex flex-1 flex-col gap-y-7">
                      ...
                      {({ open }) => (
                        <>
                          <Disclosure.Button className={classNames(
                            ...
                            <item.icon className="h-6 w-6 shrink-0" />
                            <span className={classNames(isCollapsed && 'lg:hidden', 'flex-1')}>{item.name}</span>
                            ...
                            {item.children.map((subItem) => (
                              <li key={subItem.name}>
                                <Link href={subItem.href} legacyBehavior>
                                  <a className={classNames(
                                    router.pathname === subItem.href ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800',
          ...
          <li className="-mx-6 mt-auto">
            <div onClick={() => setIsCollapsed(!isCollapsed)} className="hidden lg:flex items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-400 hover:bg-gray-800 hover:text-white cursor-pointer">
             ...
             <button onClick={switchCompany} className="w-full flex items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-gray-400 hover:bg-gray-800 hover:text-white">
                <ArrowPathIcon className="h-6 w-6 shrink-0" />
                <span className={classNames(isCollapsed && 'lg:hidden')}>Switch Company</span>
             </button>
            <button onClick={logout} className="w-full flex items-center gap-x-4 px-6 py-3 text-sm font-semibold leading-6 text-white bg-red-600 hover:bg-red-500">
      ...
      <div>
        ...
        <div className={classNames(isCollapsed ? 'lg:pl-20' : 'lg:pl-72', 'transition-all duration-300')}>
           <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
             ...
             <div className="flex-1 text-lg font-semibold leading-6 text-gray-900">{selectedCompany.name}</div>
           </div>
           <main className="py-10"><div className="px-4 sm:px-6 lg:px-8">{children}</div></main>
        </div>
      </div>
    </>
  );
